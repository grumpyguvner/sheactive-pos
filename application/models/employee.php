...
//                }
                ...
                if ($query->num_rows() ==1)
		{
			...
			return true;
                ...
                //Try logging in with 'unique' quick login (for use with employee card)
                $query = $this->db->get_where('employees', array('alt_login' => md5($username), 'deleted'=>0), 1);
                if ($query->num_rows() ==1)
		{
			$row=$query->row();
			$this->session->set_userdata('person_id', $row->person_id);
	...
	*/
	function is_logged_in()
	{
        ...
        /*
	Returns the url to display after logging in
	*/
	function login_page()
	{
		if($this->is_logged_in())
		{
                    return $this->session->userdata('login_page');
		}
