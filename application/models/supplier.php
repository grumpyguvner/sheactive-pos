...
{	
	...
	*/
	...
	/*
	Get a supplier id by company name
	*/
	function get_id_by_companyname($companyname)
	{
		$this->db->select('person_id');
		$this->db->where("company_name = '".str_replace("'", "\'", $companyname)."' and deleted=0");

		$by_name = $this->db->get('suppliers');
                foreach($by_name->result() as $row)
                {
