<?php
    ...
    {
        if($item['quantity']>0) //i.e. we are returning goods then no label required
        {
            $labelFormat .= $labelReset;
			// At the warehouse we are printing labels without prices
            $labelFormat .= $field01Format.str_pad(substr($item['name'],0,28), 28, " ", STR_PAD_BOTH)."\x0D";
            $labelFormat .= $field02Format.str_pad(substr($item['description'],0,28), 28, " ", STR_PAD_BOTH)."\x0D";
//            $labelFormat .= $field03Format.str_pad(substr($item['description'],28,56), 28, " ", STR_PAD_BOTH)."\x0D";
//            $labelFormat .= $field03Format.str_pad(substr($item['supplierref'],0,28), 28, " ", STR_PAD_BOTH)."\x0D";
