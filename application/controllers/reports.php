...
{	
	...
	function summary_discounts($start_date, $end_date, $export_excel=0)
	{
		...
		foreach($report_data as $row)
		{
			$tabular_data[] = array($row['discount_reason'],$row['count']);
		}
	...
	function graphical_summary_discounts_graph($start_date, $end_date)
	{
		...
		foreach($report_data as $row)
		{
			$graph_data[$row['discount_reason']] = $row['count'];
		}
		
		$data = array(
			"title" => $this->lang->line('reports_discounts_summary_report'),
			"yaxis_label"=>$this->lang->line('reports_count'),
			"xaxis_label"=>$this->lang->line('reports_discount_reason'),
			"data" => $graph_data
		);
...
//				$details_data[$key][] = array($drow['name'], $drow['category'], $drow['serialnumber'], $drow['description'], $drow['quantity_purchased'], to_currency($drow['subtotal']), to_currency($drow['total']), to_currency($drow['tax']),to_currency($drow['profit']), $drow['discount_percent'].'%');
				$details_data[$key][] = array($drow['name'], $drow['category'], $drow['serialnumber'], $drow['description'], $drow['quantity_purchased'], to_currency($drow['subtotal']), to_currency($drow['total']), to_currency($drow['tax']),to_currency($drow['profit']), $drow['discount']);
			}
		}
...
//				$details_data[$key][] = array($drow['name'], $drow['category'], $drow['serialnumber'], $drow['description'], $drow['quantity_purchased'], to_currency($drow['subtotal']), to_currency($drow['total']), to_currency($drow['tax']),to_currency($drow['profit']), $drow['discount_percent'].'%');
	...
	function detailed_sales($start_date, $end_date, $export_excel=0)
	{
		...
		foreach($report_data['summary'] as $key=>$row)
		{
			...
			foreach($report_data['details'][$key] as $drow)
			{
//				$details_data[$key][] = array($drow['name'], $drow['category'], $drow['serialnumber'], $drow['description'], $drow['quantity_purchased'], to_currency($drow['subtotal']), to_currency($drow['total']), to_currency($drow['tax']),to_currency($drow['profit']), $drow['discount'].$drow['discount_type']);
				$details_data[$key][] = array($drow['name'], $drow['category'], $drow['item_number'], $drow['description'], $drow['quantity_purchased'], to_currency($drow['subtotal']), to_currency($drow['total']), to_currency($drow['tax']),to_currency($drow['profit']), $drow['discount']);
			}
		}
	...
	function specific_supplier($start_date, $end_date, $supplier_id, $export_excel=0)
	{
                $this->load->model('reports/Specific_supplier');
		$model = $this->Specific_supplier;

		$headers = $model->getDataColumns();
		$report_data = $model->getData(array('start_date'=>$start_date, 'end_date'=>$end_date, 'supplier_id' =>$supplier_id));
		...
		foreach($report_data['summary'] as $key=>$row)
		{
//			$summary_data[] = array(anchor('receivings/receipt/'.$row['receiving_id'], 'RECV '.$row['receiving_id'], array('target' => '_blank')), $row['receiving_date'], $row['items_purchased'], $row['employee_name'], to_currency($row['subtotal']), to_currency($row['total']), $row['payment_type'], $row['comment']);
			$summary_data[] = array(anchor('items/view/'.$row['item_id'], $row['item_number'], array('target' => '_blank', 'class' => 'thickbox')), $row['name'], $row['description'], $row['supplierref'], $row['items_purchased'], to_currency($row['item_cost_price']), to_currency($row['total']));

			foreach($report_data['details'][$key] as $drow)
			{
//				$details_data[$key][] = array($drow['name'], $drow['category'], $drow['serialnumber'], $drow['description'], $drow['quantity_purchased'], to_currency($drow['subtotal']), to_currency($drow['total']), to_currency($drow['tax']),to_currency($drow['profit']), $drow['discount_percent'].'%');
				$details_data[$key][] = array(anchor('receivings/receipt/'.$drow['receiving_id'], 'RECV '.$drow['receiving_id'], array('target' => '_blank')), $drow['receiving_date'], $drow['quantity_purchased'], $drow['employee_name'], to_currency($drow['item_cost_price']), to_currency($drow['total'], $drow['comment']));
			}
		}
		...
		$data = array(
			...
			"summary_data" => $summary_data,
			"details_data" => $details_data,
			"overall_summary_data" => $model->getSummaryData(array('start_date'=>$start_date, 'end_date'=>$end_date,'supplier_id' =>$supplier_id)),
			"export_excel" => $export_excel
		);
	...
	function inventory_low($export_excel=0)
	{
		...
		foreach($report_data as $row)
		{
			$tabular_data[] = array($row['supplier'], $row['supplierref'], $row['name'], $row['item_number'], $row['description'], $row['quantity'], $row['reorder_level'], $row['reorder_quantity']);
		}
	...
	function inventory_summary($export_excel=0)
	{
		...
		foreach($report_data as $row)
		{
			$tabular_data[] = array($row['name'], $row['item_number'], $row['description'], $row['quantity'], $row['reorder_level'], $row['reorder_quantity']);
		}
	...
	function inventory_valuation($export_excel=0)
	{
		...
		foreach($report_data as $row)
		{
			$tabular_data[] = array($row['name'], $row['item_number'], $row['description'], $row['quantity'], $row['cost_price'], $row['value']);
		}

		$data = array(
			"title" => $this->lang->line('reports_inventory_valuation'),
			"subtitle" => '',
			"headers" => $model->getDataColumns(),
			"data" => $tabular_data,
			"summary_data" => $model->getSummaryData(array()),
