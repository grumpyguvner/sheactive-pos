...
//		$this->_reload($data);
	...
	function edit_item($line)
	{
		...
		$discount_reason = $this->input->post("discount_reason");
                if ($discount!=0){
                    $this->form_validation->set_rules('discount_reason', 'lang:sales_discount_reason', 'required');
                }
	...
	function discount_item($item_number)
	{
		...
		$this->load->view("sales/discount",$data);
	}
	...
	function discount_all()
	{
		...
		$this->load->view("sales/discount_all",$data);
	}
	...
	function capture_ean($item_number)
	{
		...
		$data['discount_type'] = $item['discount_type'];
		$data['discount_reason'] = $item['discount_reason'];
		$this->load->view("sales/capture_ean",$data);
	}
	...
	function complete()
	{
		...
		$data['employee']=$emp_info->first_name.' '.$emp_info->last_name;
                ...
                if($this->sale_lib->get_total()<>$this->sale_lib->get_payments_total()){
                    if($this->sale_lib->get_total()<0 && $this->Appconfig->get('sales_mode')=="TILL"){
			$data['error'] = $this->lang->line('sales_must_enter_refund_type');
			$data['error'] .= $this->sale_lib->get_total()."<>".$this->sale_lib->get_payments_total();
			$this->_reload($data);
			return false;
                    }
                }
                if($this->Appconfig->get('sales_mode')!="TILL"){
                    $customer_req=false;
                    $comment_req=false;
                }else{
                ...
                {
			$data['error'] = $this->lang->line('sales_must_enter_customer');
			$this->_reload($data);
			return false;
                }
		...
		if($customer_id!=-1)
		{
			...
			$data['customer']=$cust_info->first_name.' '.$cust_info->last_name;
                ...
                {
			$data['error'] = $this->lang->line('sales_must_enter_comment');
			$this->_reload($data);
			return false;
                }
...
//		$data['employee']=$emp_info->first_name.' '.$emp_info->last_name;
		...
		if($customer_id!=-1)
		{
			...
			$data['customer']=$info->first_name.' '.$info->last_name;
                ...
                if($this->Appconfig->get('sales_mode')=="TILL"){
                    ...
                    $this->load->view("sales/internet",$data);
                }
	}
	...
	$prefix=$this->Appconfig->get('receipt_prefix');
        ...
        $this->load->view("sales/no_sale",$data);
        $this->sale_lib->clear_all();
