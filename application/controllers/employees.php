...
class Employees extends Person_controller
{
	...
	*/
	function save($employee_id=-1)
	{
		...
		//Password has been changed OR set for the first time
		if($this->input->post('password')!='')
		{
			$employee_data['password']=md5($this->input->post('password'));
		}
		//Alt. login has been changed OR set for the first time
		if($this->input->post('alt_login')!='')
		{
			$employee_data['alt_login']=md5($this->input->post('alt_login'));
		}
